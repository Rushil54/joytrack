...
int main() {
    ...
    while (true) {
        if (XInputGetState(0, &state) == ERROR_SUCCESS) {
            ...
            // X and Y Buttons
            if (xPressed) keyPress(VK_UP); // Maximize Window (Win+Up)
            if (yPressed) keyPress(0x57); // Close Tab (Ctrl+W)
...
//dpad left riht only working in yt not in media
//x and y not working
//win+tab notworking
//only this issues rest all fine
