...
void clickMouse(DWORD flag) {
    ...
    input.mi.dwFlags = (flag == MOUSEEVENTF_LEFTDOWN) ? MOUSEEVENTF_LEFTUP : 
...
int main() {
    ...
    while (true) {
        if (XInputGetState(0, &state) == ERROR_SUCCESS) {
            ...
            // Speed Boost Toggle moved to Y button
            speedBoost = yPressed;
            ...
            if (xPressed && !prevX) clickMouse(MOUSEEVENTF_MIDDLEDOWN); // Middle Mouse Button
            prevA = aPressed;
            prevB = bPressed;
            prevX = xPressed;
